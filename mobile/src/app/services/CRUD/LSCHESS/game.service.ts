...
export class GameService {
   ...
   url = environment.api_lschess + 'game/';
   ...
   post(game: Game): Promise<any> {
      return this.http.post(this.url, JSON.stringify(game), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(game: Game): Promise<any> {
      return this.http.put(this.url, JSON.stringify(game), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
