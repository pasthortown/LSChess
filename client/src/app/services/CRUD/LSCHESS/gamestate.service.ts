...
export class GameStateService {
   ...
   url = environment.api_lschess + 'gamestate/';
   ...
   post(gamestate: GameState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(gamestate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(gamestate: GameState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(gamestate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
